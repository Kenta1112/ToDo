  ...
  if(empty($err_msg)){
    ...
    try{
      ...
      if($stmt){
        ...
        //セッション関係
        //セッション有効期限(default=1hour)
        $sessionLimit=60*60;
        //ログイン日時
        $_SESSION['login_date']=time();
        debug('ユーザ登録日時'.print_r($_SESSION['login_date'],true));
        //セッション有効期限
        $_SESSION['login_limit']=$sessionLimit;
        debug('セッション有効期限'.print_r($_SESSION['login_limit'],true));
        //セッションのユーザーID
        $_SESSION['user_id']=$dbh->lastInsertId();
        debug('セッションのユーザーID'.print_r($_SESSION['user_id'],true));
...
<p><span class="err_msg"><?php err_msg('common');?></span></p>
<p><span >Email</span><span class="err_msg"><?php err_msg('email');?></span></p>
<input type="text" name="email">
<p><span>パスワード</span><span class="err_msg"><?php err_msg('password');?></span></p>
<input type="password" name="password">
<p><span>パスワード再入力</span><span class="err_msg"><?php err_msg('pass_retype');?></span></p>
